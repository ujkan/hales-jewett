...
text \<open>For any function $f$ whose image under a set $A$ is a subset of another set $B$, there's a unique function $g$ in the function space $B^A$ that equals $f$ everywhere in $A$.
The function $g$ is usually written as $f|_A$ in the mathematical literature.\<close>
...
text \<open>A line $L$ in the $n$-dimensional cube

\begin{tabular}{lcp{8cm}}
$n$:& \<open>nat\<close>& dimension\\
$t$:&  \<open>nat\<close>& the size of the base\\
\end{tabular}\<close>
...
text \<open>We introduce an elimination rule to relate lines with the more general definition of a subspace (see below). \<close>
lemma is_line_elim_t_1:
  assumes "is_line L n t" and "t = 1"
  obtains B\<^sub>0 B\<^sub>1
  where "B\<^sub>0 \<union> B\<^sub>1 = {..<n} \<and> B\<^sub>0 \<inter> B\<^sub>1 = {} \<and> B\<^sub>0 \<noteq> {} \<and> (\<forall>j \<in> B\<^sub>1. (\<forall>x<t. \<forall>y<t. L x j = L y j)) \<and> (\<forall>j \<in> B\<^sub>0. (\<forall>s<t. L s j = s))"
proof -
  ...
  moreover have "(\<forall>j \<in> B0. (\<forall>s<t. L s j = s))" using assms(1, 2) cube1_alt_def unfolding B0_def is_line_def by auto
...
proof -
  ...
  moreover have "(restrict (\<lambda>y. L (y 0)) (cube 1 1)) \<in> cube 1 1 \<rightarrow>\<^sub>E cube n 1" using assms(2) cube1_alt_def unfolding is_line_def by auto
  moreover have "(\<forall>y\<in>cube 1 1. (\<forall>i\<in>B 1. (restrict (\<lambda>y. L (y 0)) (cube 1 1)) y i = f i) \<and> (\<forall>j<1. \<forall>i\<in>B j. (restrict (\<lambda>y. L (y 0)) (cube 1 1)) y i = y j))" using cube1_alt_def B_props * unfolding B_def f_def by auto
...
proof-
  ...
  proof
    ...
    moreover have "(SOME p. p\<in>cube 1 t \<and> p 0 = s) \<in> cube 1 t" using cube_props(1) a someI_ex[of "\<lambda>p. p \<in> cube 1 t \<and> p 0 = s"] by blast
...
proof -
  ...
  then have "\<chi> (S (the_inv_into (cube 1 t) (\<lambda>f. f 0) s)) = c1" if "s < t" for s using one_dim_cube_eq_nat_set[of t] 
    by (meson that bij_betwE bij_betw_the_inv_into lessThan_iff)
  then have K1: "\<chi> (S (SOME p. p\<in>cube 1 (t+1) \<and> p 0 = s)) = c1" if "s < t" for s using that some_inv_into_2 by simp
  ...
  moreover have "\<exists>!x \<in> cube 1 (t+1). x 0 = t" using one_dim_cube_eq_nat_set[of "t+1"] unfolding bij_betw_def inj_on_def 
    using inv_into_cube_props(1) inv_into_cube_props(2) by force 
  moreover have **: "\<exists>!x. x  \<in> classes 1 t 1" unfolding classes_def using calculation(2) by simp
  ultimately have "the_inv_into (cube 1 (t+1)) (\<lambda>f. f 0) t \<in> classes 1 t 1" using inv_into_cube_props[of t "t+1"] unfolding classes_def by simp
...
lemma hj_imp_lhj_base: 
  ...
  shows "lhj r t 1"
proof-
  from assms(2) obtain N where N_def: "N > 0 \<and> (\<forall>N' \<ge> N. \<forall>\<chi>. \<chi> \<in> (cube N' t) \<rightarrow>\<^sub>E {..<r::nat} \<longrightarrow> (\<exists>L. \<exists>c<r. is_line L N' t \<and> (\<forall>y \<in> L ` {..<t}. \<chi> y = c)))" unfolding hj_def by blast

  have "(\<exists>S. is_subspace S 1 N' (t + 1) \<and> (\<forall>i \<in> {..1}. \<exists>c < r. (\<forall>x \<in> classes 1 t i. \<chi> (S x) = c)))" if asm: "N' \<ge> N" "\<chi> \<in> (cube N' (t + 1)) \<rightarrow>\<^sub>E {..<r::nat}" for N' \<chi>
  proof-
    have N'_props: "N' > 0 \<and> (\<forall>\<chi>. \<chi> \<in> (cube N' t) \<rightarrow>\<^sub>E {..<r::nat} \<longrightarrow> (\<exists>L. \<exists>c<r. is_line L N' t \<and> (\<forall>y \<in> L ` {..<t}. \<chi> y = c)))" using asm N_def by simp
    ...
    proof-
      ...
      proof-
        ...
        proof (cases)
          case 1
          then have "(restrict (\<lambda>y. L (y 0)) (cube 1 t)) y j = f j" if "y \<in> cube 1 t" for y using that Bf_defs by simp
          moreover have "\<exists>!i. i < t \<and> y 0 = i" if "y \<in> cube 1 t" for y using that one_dim_cube_eq_nat_set[of "t"] unfolding bij_betw_def by blast
          moreover have "\<exists>!y. y \<in> cube 1 t \<and> y 0 = i" if "i < t" for i 
          proof (intro ex1I_alt)
            define y where "y \<equiv> (\<lambda>x::nat. \<lambda>y\<in>{..<1::nat}. x)" 
    ...
    qed
    then have F1: "is_subspace S1 1 N' (t + 1)" unfolding S1_def using line_is_dim1_subspace[of "N'" "t+1"] N'_props assms(1) by force
    ...
    proof-
      ...
      next
        ...
        have "\<forall>s \<in> {..<t+1}. \<exists>!x \<in> cube 1 (t+1). (\<lambda>p. \<lambda>y\<in>{..<1::nat}. p) s = x" using nat_set_eq_one_dim_cube[of "t+1"] 
        ...
        proof (intro ex1I_alt)
          ...
          proof (rule ccontr)
            ...
            qed
          qed
          ultimately show "\<exists>y. (y \<in> cube 1 (t + 1) \<and> y 0 = t) \<and> (\<forall>ya. ya \<in> cube 1 (t + 1) \<and> ya 0 = t \<longrightarrow> y = ya)" by blast
        qed
        ...
        proof-
          ...
          moreover have "S1 x \<in> cube N' (t+1)" unfolding S1_def is_line_def using line_prop line_points_in_cube redef x_def by fastforce
    ...
    qed
    ultimately show "(\<exists>S. is_subspace S 1 N' (t + 1) \<and> (\<forall>i \<in> {..1}. \<exists>c < r. (\<forall>x \<in> classes 1 t i. \<chi> (S x) = c)))" by blast
  qed
...
proof-
  ...
  proof -
    ...
    have line_subspace_s: "\<exists>S. layered_subspace S 1 n t s \<chi> \<and> is_line (\<lambda>s\<in>{..<t+1}. S (SOME p. p\<in>cube 1 (t+1) \<and> p 0 = s)) n (t+1)" if "\<chi> \<in> (cube n (t + 1)) \<rightarrow>\<^sub>E {..<s::nat}" for \<chi> 
    proof-
    ...
    have "card (cube m (t+1) \<rightarrow>\<^sub>E {..<r}) = (card {..<r}) ^ (card (cube m (t+1)))"  using card_PiE[of "cube m (t + 1)" "\<lambda>_. {..<r}"] by (simp add: cube_def finite_PiE)
    ...
    moreover have F5: "((\<forall>i \<in> BT (k + 1). T y i = fT i) \<and> (\<forall>j<k+1. \<forall>i \<in> BT j. (T y) i = y j))" if "y \<in> cube (k + 1) (t + 1)" for y
    proof(intro conjI allI impI ballI)
      ...
      next
        ...
        next
          ...
          moreover have "p u = restrict y {..<1} u" if "u \<notin> {..<1}" for u using that calculation XX unfolding cube_def using PiE_arb[of "restrict y {..<1}" "{..<1}" "\<lambda>x. {..<t + 1}" u]  PiE_arb[of p "{..<1}" "\<lambda>x. {..<t + 1}" u] by simp
    ...
    proof
      ...
      proof
        ...
        moreover have "\<forall>u < k - j + 1. p u \<noteq> t" using calculation by (auto simp: algebra_simps less_Suc_eq_0_disj)
    ...
    next
      ...
      proof
        ...
        have "(\<lambda>g \<in> {..<1}. y g) \<in> cube 1 (t+1)" using y_prop classes_subset_cube[of "k+1" t j] cube_restrict[of 1 "(k+1)" y "t+1"] assms(2) by auto
    ...
    proof-
      ...
      moreover have **: "T_class i = {join (L_line l) s n m | l s . l \<in> {..<t} \<and> s \<in> S ` (classes k t i)}" using a unfolding T_class_def by simp
      ...
      then have key_aux: "\<chi>L_s (L_line s) = \<chi>L_s (L_line l)" if "s \<in> {..<t}" "l \<in> {..<t}" for s l using that  unfolding L_line_def 
      ...
      then show " \<chi> x = \<chi> y \<and> \<chi> x < r" using xdefs AA1 key assms(1) A \<open>L_line xi \<in> cube n (t + 1)\<close> \<open>xs \<in> cube m (t + 1)\<close> by blast
    qed
    ...
    proof -
      ...
      then have "\<exists>!y. y \<in> classes (k+1) t (k+1)" using \<open>\<forall>x \<in> classes (k+1) t (k+1). \<forall>u < k + 1. x u = t\<close> by auto
    ...
    qed
    ultimately have "\<exists>c<r. \<forall>x \<in> T ` classes (k+1) t i. \<chi> x = c" if "i \<le> k + 1" for i using that by (metis Suc_eq_plus1 le_Suc_eq)
    then have "\<exists>c<r. \<forall>x \<in> classes (k+1) t i. \<chi> (T x) = c" if "i \<le> k + 1" for i using that by simp
...
proof (induction k arbitrary: r rule: less_induct)
  ...
  next
    ...
    next
      ...
      next
        case 2
        then obtain N where N_props: "N > 0" "\<forall>N'\<ge>N. \<forall>\<chi> \<in> cube N' t \<rightarrow>\<^sub>E {..<r}. (\<exists>L c. c < r \<and> is_line L N' t \<and> (\<forall>y \<in> L ` {..<t}. \<chi> y = c))" using assms[of r] unfolding hj_def by force
...
proof-
  ...
  proof -
    ...
    qed
    then have "x i1 \<noteq> x i2" if "i1 \<le> k" "i2 \<le> k" "i1 \<noteq> i2" "i1 < i2" for i1 i2 using that by fastforce
  ...
  also have " ... = card (S ` x ` {..k})" using subspace_inj_on_cube[of S k n "t+1"] card_image[of S "x ` {..k}"] inj_on_subset[of S "cube k (t+1)" "x ` {..k}"]  assms(1) \<open>x ` {..k} \<subseteq> cube k (t + 1)\<close> unfolding layered_subspace_def by simp
  ...
  then obtain u v where uv_props: "u \<in> {..k} \<and> v \<in> {..k} \<and> u < v \<and> \<chi> (S (x u)) = \<chi> (S (x v))" by (metis linorder_cases)
  ...
  ultimately show "\<exists>L c. c < k \<and> is_line L n (t + 1) \<and> (\<forall>y\<in>L ` {..<t + 1}. \<chi> y = c)" by blast
...
lemma single_point_line_is_monochromatic: 
  ...
  shows "(\<exists>c < r. is_line (\<lambda>s\<in>{..<1}. \<lambda>a\<in>{..<N}. 0) N 1 \<and> (\<forall>i \<in>  (\<lambda>s\<in>{..<1}. \<lambda>a\<in>{..<N}. 0) ` {..<1}. \<chi> i = c))"
proof -
  have "is_line (\<lambda>s\<in>{..<1}. \<lambda>a\<in>{..<N}. 0) N 1" using assms(2) single_point_line by blast
  moreover have "\<exists>c < r. \<chi> ((\<lambda>s\<in>{..<1}. \<lambda>a\<in>{..<N}. 0) j) = c" if "(j::nat) < 1" for j using assms line_points_in_cube calculation that unfolding cube_def by blast
...
text \<open>Any cube over 1 element always has a single point, which also forms the only line in the cube. Since it's a single point line, it's trivially monochromatic. We show the result for dimension 1.\<close>
lemma hj_t_1: "hj r 1"
  unfolding hj_def 
proof-
  let ?N = 1
  have "\<exists>L c. c < r \<and> is_line L N' 1 \<and> (\<forall>y\<in>L ` {..<1}. \<chi> y = c)" if "N' \<ge> ?N" "\<chi> \<in> cube N' 1 \<rightarrow>\<^sub>E {..<r}" for N' \<chi> using single_point_line_is_monochromatic[of \<chi> N' r] that by force
